...
from music21 import (chord, clef, converter, duration, instrument, metadata,
...
def detect_notes_and_chords(audio_data, sample_rate):
    ...
    onsets = librosa.onset.onset_detect(
        ...
        onset_envelope=np.where(
            onset_strength >= 0.85, onset_strength, 0)
    )
...
def visualize_audio(audio_data, sample_rate, output_image=None):
    ...
    onset_strength = librosa.onset.onset_strength(
        y=audio_data[:30*44100], sr=sample_rate, hop_length=512)
